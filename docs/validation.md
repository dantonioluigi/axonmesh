...
`yolosplit evaluate --bottleneck` — baseline vs split+bottleneck mAP on the same
...
```bash
yolosplit train-bottleneck --model yolo11n.pt \
...
python -c "from yolosplit.evaluate import compare_map; \
