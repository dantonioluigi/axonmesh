...
For a YOLO11l detector fine-tuned on a fixed industrial camera scene, shipping
the INT8-quantised
...
1. **Inspect** — `axonmesh inspect --model yolo11l.pt`. Confirms the
...
2. **Measure bandwidth** — `axonmesh measure` on a representative sample of
...
3. **Measure accuracy** — `axonmesh evaluate` on the validation split: ultralytics `val()` twice, unsplit baseline vs split with the INT8
...
- Adaptive policy: confidence high → ship detections only; medium → ship
  ...
  retraining dataset (the next dataset version, built from hard cases).
